...
//		// ACCMAG(9), POT(10), ELE(11), SLP(12), AREA(13);
//
//		private int value;
//
...
//				sb.append(sep).append(String.format("%.16f", s));
//			}
//			sep = separator;
//		}
//		return sb.toString();
...
//		cols.add("Radius (km)");
//		cols.add(AltwgProductType.NORMAL_VECTOR_X.getHeaderValueWithUnits());
//		cols.add(AltwgProductType.NORMAL_VECTOR_Y.getHeaderValueWithUnits());
//		cols.add(AltwgProductType.NORMAL_VECTOR_Z.getHeaderValueWithUnits());
//
//		// area is written out to ascii file but not to DTM plane! That is taken care
//		// of by Shape2Tilt!
//		cols.add("Area (km^2)");
//		cols.add(AltwgProductType.GRAVITY_VECTOR_X.getHeaderValueWithUnits());
//		cols.add(AltwgProductType.GRAVITY_VECTOR_Y.getHeaderValueWithUnits());
//		cols.add(AltwgProductType.GRAVITY_VECTOR_Z.getHeaderValueWithUnits());
//		cols.add(AltwgProductType.GRAVITATIONAL_MAGNITUDE.getHeaderValueWithUnits());
//		cols.add(AltwgProductType.GRAVITATIONAL_POTENTIAL.getHeaderValueWithUnits());
//		cols.add(AltwgProductType.ELEVATION.getHeaderValueWithUnits());
//		cols.add(AltwgProductType.SLOPE.getHeaderValueWithUnits());
//
//		String dl = ",";
...
//					.format("%s -d %.16e -r %.16e --%s --%s --start-index %d --end-index %d --suffix %s%d "
//							+ "--output-folder %s %s", rootDir + File.separator + gravityExecutableName,	//was between %s and %s /*--gravConst %.16e*/
...
//
//		/*
//		 * Configure batch to run in specified output folder if using grid engine,
//		 * otherwise otherwise set to null to allow local OS to write files in local temp folder.
//		 * Explicitly setting batchDir for grid engine to get around permission problems where
//		 * a node may not have permission to write in the local /tmp folder
//		 */
...
//		// Otherwise abort.
//		System.out.println("SBMTDistributedGravity: main: arg files count " + arg.files.size());
...
//
//		rootDir = filenames.get(2);
//	    gravityExecutableName = filenames.get(3);
//
...
//
//		System.out.println(StringUtil.timenow() + ":starting SBMTDistributedGravity");
//
...
//		saveResultsAtCenters(tableFile, fitspolydata, gravAtLocations);
//
...
//		dataBuilder.setAltProdType(AltwgProductType.DTM);
...
//		// planeList.add(PlaneInfo.ELEV_NORM);
...
//		System.out.println("SBMTDistributedGravity: saveLocalFitsCenters: number of cells " + numCells);
//
...
//
//		double[][] tableResults = new double[3 + numGravPlanes][numCells];
//
...
//			double elevation = getElevation(refPotential, gravAtLocations.get(ii).acc,
//					gravAtLocations.get(ii).potential);
//			double accMag = getAccelerationMagnitude(gravAtLocations.get(ii).acc, slope);
//
//			// get grav magnitude, potential, elevation, slope
//			tableResults[GridIndex.ACCMAG.index()][ii] = accMag;
//			tableResults[GridIndex.POT.index()][ii] = gravAtLocations.get(ii).potential;
//			tableResults[GridIndex.ELE.index()][ii] = elevation;
//			tableResults[GridIndex.SLP.index()][ii] = slope;
//
//			// get area
//			// tableResults[GridIndex.AREA.index()][ii] = ci.center[1];
//
//		}
//
//		return tableResults;
